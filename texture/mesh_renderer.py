...
class MeshRenderer:
    ...
    def render_ndc(self, mesh_dict):
        ...
        max_faces_per_bin = int(faces.shape[1])
    ...
    def render_mesh(self, mesh_dict, cam_int, cam_ext):
        ...
        # in Pytorch3D, x to left, y to topy, z to front
        # in OPENCV, x to right, y to down, z to front
        # here we flip x and y to fit the OPENCV format to the Pytorch3D format
        vertice_ndc[..., :-1] *= -1
        if h > w:
            vertice_ndc[..., 1] *= h / w
        else:
